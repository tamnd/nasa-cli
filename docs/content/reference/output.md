...
describes what users get. Pick a format with `-o`, or let nasa choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
nasa <command> -o table   # aligned columns for reading
nasa <command> -o jsonl   # one JSON object per line, for piping
nasa <command> -o json    # a single JSON array
nasa <command> -o csv     # spreadsheet friendly
nasa <command> -o tsv     # tab-separated
nasa <command> -o url     # just the URL column
nasa <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
nasa <command> --fields id,title,url
```
...
```bash
nasa <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
nasa <command>            # a table, because this is a terminal
nasa <command> | wc -l    # JSONL, because this is a pipe
```
